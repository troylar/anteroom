...
@pytest.mark.asyncio
async def test_textual_backend_submit_turn_non_retryable_error_persists_no_assistant_message(tmp_path) -> None:
    db = init_db(tmp_path / "textual_submit_turn_error.db")
    ai_service = SimpleNamespace(
        config=SimpleNamespace(model="gpt-5.2", narration_cadence="compact"),
        generate_title=lambda prompt: asyncio.sleep(0, result=f"Title: {prompt[:20]}"),
    )
    ...
    async def _fake_run_agent_loop(**_: Any):
        ...
        yield AgentEvent(
            kind="tool_call_end",
            data={"tool_name": "grep", "status": "error", "output": {"error": "permission denied"}},
        )
        yield AgentEvent(
            kind="error",
            data={"message": "Permission denied", "retryable": False},
        )
    ...
    assert tool_entry["output"] == {"error": "permission denied"}
