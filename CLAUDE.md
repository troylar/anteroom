...
#### Entry Points & Core
- **`__main__.py`** — Argparse dispatch: `init`, `config`, `chat`, `exec`, `db`, `usage`, `audit`, `artifact`, `pack`, `space`, `start`, `stop`, `status` subcommands. Global flags: `--version`, `--test` (AI connection + knowledge pipeline deps), `--allowed-tools`, `--denied-tools`, `--approval-mode`, `--port`, `--debug`, `--team-config`. Chat flags: `--trust-project`, `--no-project-context`, `--plan`. Audit flags: `audit {verify,purge}`. DB flags: `db {list,show,purge,encrypt}` for data retention. Config flags: `config {validate,view}` with `--with-sources` for layer annotation. Artifact flags: `artifact {list,show,check,import,create}` with type/namespace/source filters; `--fix`, `--json-output`, `--instructions` for various commands. Pack flags: `pack {list,install,show,remove,update,sources,refresh,attach,detach,add-source}` with `--project` for attach/detach to scope attachment to the current space; `pack install` accepts git URLs (`https://`, `ssh://`, `git@host:path`) for direct repo install with `--branch`, `--path`, `--attach`, `--priority` flags; `--attach` auto-attaches after install; `--priority` (1-100, default 50) sets conflict resolution precedence (lower wins). Pack commands work without `config.yaml` for zero-config onboarding. Space flags: `space {list,create,init,load,show,delete,refresh,clone,map,move-root}` with `--space` on chat to force a specific space. `space init` auto-derives space name from cwd and creates `.anteroom/space.yaml` in the current project. Server management: `start` (launch background server, `--no-browser`), `stop` (graceful shutdown), `status` (PID, port, uptime). Hidden `--_bg-worker` flag for internal use by the background child process
...
- **`config.py`** — YAML config loader with layered precedence: defaults < team < **packs** < personal < space < env vars < CLI flags. `load_config()` accepts optional `pack_config` dict (pre-merged pack overlays) applied between team and personal layers. Dataclass hierarchy: `AppConfig` → `AIConfig`, `AppSettings`, `CliConfig`, `PlanningConfig`, `SkillsConfig`, `McpServerConfig`, `SafetyConfig`, `SubagentConfig`, `EmbeddingsConfig`, `UsageConfig`, `ProxyConfig`, `ReferencesConfig`, `RagConfig`, `RerankerConfig`, `CodebaseIndexConfig`, `SessionConfig`, `StorageConfig`, `AuditConfig`, `DlpConfig`, `PackSourceConfig`. Enforces locked fields from team `enforce` list. Config validated via `services/config_validator.py` before parsing
...
- **`services/agent_loop.py`** — Shared agentic loop: streams responses, parses tool calls, parallel execution via `asyncio.as_completed`, max 50 iterations. Cancel-aware. Auto-compacts at configurable token threshold. Supports prompt queuing, narration cadence, auto-plan threshold. Emits `tool_batch_start` and `tool_batch_end` events around parallel tool execution (with `call_count` and `elapsed_seconds`). Consecutive text-only response limit prevents runaway loops (configurable, default 3). Intra-response line repetition detection stops degenerate LLM output within a single streaming response (configurable, default 5 repeated lines). Consecutive identical tool batch detection stops runaway loops when the same tool(s) are called with identical arguments repeatedly (`max_identical_tool_repeats`, default 3, 0 to disable). DLP scanning on streamed chunks and final assembled text (with `dlp_blocked` and `dlp_warning` events). Output content filtering with system prompt leak detection (with `output_filter_blocked` and `output_filter_warning` events). Internal `_`-prefixed metadata keys stripped before sending to LLM
...
- **`services/storage.py`** — SQLite DAL with parameterized queries, UUID IDs. Vector storage via usearch (graceful degradation). Source CRUD, tags, groups, space CRUD and linking, text chunking, embeddings. Token usage tracking. Message metadata (JSON) for RAG provenance. Scoped semantic search with iterative widening. FTS5 keyword search. `reprocess_source()` re-extracts from stored files. `get_source_embedding_status()` for embedding health
...
- **`services/context_trust.py`** — Prompt injection defense: trusted/untrusted classification, defensive XML envelopes with origin attribution, sanitization against envelope breakout
...
- **`services/artifact_storage.py`** — Artifact CRUD against SQLite. Parameterized queries, JSON metadata, version auto-bump on content change. `list_artifacts()` accepts `attached_only` and `space_id` kwargs — when `attached_only=True`, JOINs with `pack_artifacts` and `pack_attachments` to exclude unattached pack artifacts and config overlays
- **`services/artifact_registry.py`** — In-memory artifact index with 6-layer precedence (built_in < global < team < project < local < inline). `MAX_ARTIFACTS` cap (500). `load_from_db()` accepts optional `space_id` kwarg, passes `attached_only=True` to load only artifacts from attached packs (standalone artifacts always included)
...
- **`services/starter_packs.py`** — Built-in pack templates from `packs/<name>/pack.yaml`. Starter packs (python-dev, security-baseline) auto-install on first run. Example packs (code-review, writing-assistant, strict-safety) ship with Anteroom but require manual install. `list_starter_packs()`, `list_example_packs()`, `list_all_built_in_packs()`, `get_built_in_pack_path()`
- **`services/rule_enforcer.py`** — Hard rule enforcement at the tool execution layer. Rules with `enforce: hard` metadata are checked before every tool call. `RuleEnforcer` class loads `ParsedRule` entries from rule artifacts, matches tool name and regex patterns against stringified arguments. Returns `(blocked, reason, rule_fqn)` tuple. Pure functions + one stateful class, no I/O except in-memory artifact registry
- **`services/pack_attachments.py`** — Pack attachment tracking at global/space scopes with priority-based conflict resolution (1-100, lower wins). Conflict detection for config overlays at attach time
...
#### Web UI (routers/)
- **`routers/chat.py`** — SSE chat streaming with dataclass-based architecture: `ChatRequestContext`, `WebConfirmContext`, `ToolExecutorContext`, `StreamContext`, `SourceResolutionResult`. Extracted functions: `_parse_chat_request()`, `_resolve_sources()`, `_build_tool_list()`, `_build_chat_system_prompt()`, `_web_confirm_tool()`, `_execute_web_tool()`, `_stream_chat_events()`, `_get_request_registries()`. `_get_request_registries()` builds per-request space-scoped artifact registry, skill registry, and rule enforcer. `ToolExecutorContext` carries a `rule_enforcer` field passed as `rule_enforcer_override` to `check_safety()` and `call_tool()` for concurrency-safe rule enforcement across overlapping requests. `_build_chat_system_prompt()` returns `(str, dict)` tuple — prompt text and metadata dict (`rag_status`, `rag_chunks`, `rag_sources`, `sources_truncated`). `prompt_meta` SSE event emits metadata to web UI. `user_message` SSE event emits `{id, position}` for client-side action button attachment. `done` event enriched with `assistant_message_id` and `assistant_message_position`. `StreamContext.user_msg` carries the stored user message dict. `_resolve_sources()` returns `SourceResolutionResult` with content, exclusion metadata, and truncation tracking; enforces space scoping. Supports prompt queuing (max 10), source injection (50K char limit), plan mode, sub-agents, skill registry, attachment guidance
...
- **`routers/chat.py`** — SSE chat streaming with dataclass-based architecture: `ChatRequestContext`, `WebConfirmContext`, `ToolExecutorContext`, `StreamContext`. Extracted functions: `_parse_chat_request()`, `_resolve_sources()`, `_build_tool_list()`, `_build_chat_system_prompt()`, `_web_confirm_tool()`, `_execute_web_tool()`, `_stream_chat_events()`, `_get_request_registries()`. `_get_request_registries()` builds per-request space-scoped artifact registry, skill registry, and rule enforcer. `ToolExecutorContext` carries a `rule_enforcer` field passed as `rule_enforcer_override` to `check_safety()` and `call_tool()` for concurrency-safe rule enforcement across overlapping requests. `_build_chat_system_prompt()` returns `(str, dict)` tuple — prompt text and metadata dict (`rag_status`, `rag_chunks`, `rag_sources`, `sources_truncated`). `prompt_meta` SSE event emits metadata to web UI. `user_message` SSE event emits `{id, position}` for client-side action button attachment. `done` event enriched with `assistant_message_id` and `assistant_message_position`. `StreamContext.user_msg` carries the stored user message dict. `_resolve_sources()` enforces space scoping via `_allowed_ids` for direct, tag, and group source resolution. Supports prompt queuing (max 10), source injection (50K char limit), plan mode, sub-agents, skill registry integration (injects `invoke_skill` tool and `<available_skills>` catalog into system prompt), attachment guidance (injects filenames and "use extracted content" instruction into system prompt when files are attached, sanitized via `sanitize_trust_tags()`). Forwards `tool_batch_start` and `tool_batch_end` agent events as SSE events for web UI batch grouping
- **`routers/sources.py`** — Sources API: CRUD, file upload, tags, groups, space linking, `POST /sources/{id}/reprocess` for re-extraction and re-chunking
...
#### CLI Modules
- **`cli/repl.py`** — Main REPL loop with prompt_toolkit. Orchestrates: system prompt building, space context, trust verification, plan mode, skill auto-invocation. Delegates slash commands to `commands.py`, agent turns to `agent_turn.py`, events to `event_handlers.py`. Manages `/pack`, `/space`, `/artifact`, `/reprocess` commands. `/instructions` alias for `/conventions`. Space auto-detection from cwd with parent walk-up. Skill args sanitized with `sanitize_trust_tags()` and delimited with `<skill_args>` tags for injection defense
- **`cli/repl.py`** — Main REPL loop with prompt_toolkit. Orchestrates: system prompt building, space context, trust verification, plan mode, skill auto-invocation. Delegates slash commands to `commands.py`, agent turns to `agent_turn.py`, events to `event_handlers.py`. Manages `/pack`, `/space`, `/artifact`, `/reprocess` commands. `/instructions` alias for `/conventions`. Space auto-detection from cwd with parent walk-up. Skill args sanitized with `sanitize_trust_tags()` and delimited with `<skill_args>` tags for injection defense. Ctrl-O keybinding expands/collapses the most recent tool batch fold
...
- **`cli/renderer.py`** — Rich terminal output: thinking spinner, plan checklist, inline diffs, tool call dedup, subagent rendering, and RAG status feedback. Focus & Fold adds turn-level tool batch grouping with an in-place progress ticker (`FoldGroup`, `_update_fold_ticker()`), compact completion summaries, and Ctrl-O expand/collapse. `use_stdout_console()` provides REPL-compatible mode via the raw stderr fd. `write_raw()` bypasses `patch_stdout` for approval and `ask_user` sub-prompts.
...
- **`cli/skills.py`** — Skills registry: loads YAML from `cli/default_skills/` (built-in), `~/.anteroom/skills/` (global), `.anteroom/skills/` or `.claude/skills/` (space-scoped). `Skill` dataclass includes `namespace` field for pack-sourced skills. `SkillRegistry` with namespace-aware resolution: skills stored by canonical key (`namespace/name` or bare `name`); unique bare names resolve without namespace prefix, ambiguous names require `namespace/name`. Methods: `load()`, `reload()`, `resolve_input()`, `load_from_artifacts()`. Name validation, collision detection, `MAX_SKILLS` (100), `MAX_PROMPT_SIZE` (50KB). Auto-invocation via synthetic `invoke_skill` tool. Artifact bridge: `load_from_artifacts(artifact_registry)` imports skill-type artifacts (filesystem skills take precedence)
...
SQLite with WAL journaling, FTS5 for search, foreign keys enforced. Optional SQLCipher encryption at rest (via `encrypt_at_rest` config). Schema in `db.py`. `init_db()` signature: `init_db(db_path, vec_dimensions=384, encryption_key=None)`. Key tables: conversations, messages, messages_fts/source_chunks_fts (FTS5 for hybrid RAG keyword search), tool_calls, sources/source_chunks/source_tags/source_groups, canvases, message_embeddings, source_chunk_embeddings, artifacts, artifact_versions, packs, pack_artifacts, spaces, space_paths, space_sources, pack_attachments. Vector similarity via usearch (graceful degradation). Retention worker cascades deletes.
...
- **`CliConfig`** — Context compaction thresholds, tool dedup, retry behavior, visual thresholds, consecutive text-only loop limit (`max_consecutive_text_only`, default 3, 0 to disable), intra-response line repetition limit (`max_line_repeats`, default 5, 0 to disable), consecutive identical tool batch limit (`max_identical_tool_repeats`, default 3, 0 to disable)
