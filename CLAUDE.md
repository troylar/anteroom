...
- **`services/storage.py`** — SQLite DAL with parameterized queries, UUID IDs. Vector storage via usearch (graceful degradation). Source CRUD, tags, groups, space CRUD and linking, text chunking, embeddings. Token usage tracking. Scoped semantic search with iterative widening
...
- **`services/rag.py`** — RAG pipeline: embed query, search usearch indexes, filter/deduplicate/trim to token budget. Space-scoped retrieval, conversation type labels, graceful degradation
