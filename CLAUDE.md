...
# Testing
pytest tests/ -v                    # All tests (~960 tests)
...
- **`tools/tiers.py`** — Tool risk tier system (Claude Code-style). `ToolTier` enum: READ (0), WRITE (1), EXECUTE (2), DESTRUCTIVE (3). `ApprovalMode` enum: AUTO (bypass all), ASK_FOR_DANGEROUS (default, only destructive), ASK_FOR_WRITES (write+execute+destructive), ASK (same as ask_for_writes). `should_require_approval()` returns tri-state: True (needs approval), False (auto-allow), None (hard deny from denied_tools). `DEFAULT_TOOL_TIERS` maps built-in tools; unknown/MCP tools default to EXECUTE
