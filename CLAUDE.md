...
- **`services/config_overlays.py`** — Pack config overlay collection, merging, and conflict detection. `collect_pack_overlays()` reads `config_overlay` artifacts from DB for attached packs. `merge_pack_overlays()` combines overlays sorted by priority (lower number wins). `detect_overlay_conflicts()` finds dot-path collisions — same priority + same key is an error, different priorities are allowed. `detect_artifact_conflicts()` checks exclusive artifact types (`skill`) for name collisions across packs; additive types (`rule`, `instruction`, `context`, `memory`, `mcp_server`) can coexist. `flatten_to_dot_paths()` converts nested dicts to flat keys for set-intersection comparison. `track_config_sources()` builds `{dot.path: layer_name}` map for `aroom config view --with-sources`. `check_enforced_field_violations()` pre-flight validation for team-enforced fields
...
- **`services/pack_attachments.py`** — Tracks active packs at global/space scopes with priority-based conflict resolution. `attach_pack()` accepts `priority` (1-100, default 50; lower wins). Conflict detection at attach time: config overlay dot-path collisions (same priority = error, different priority = allowed) and exclusive artifact name collisions (`skill`). `get_active_pack_ids()` and `get_attachment_priorities()` for overlay merge ordering. Syncs with rules/skills loading
