...
- **`services/storage.py`** — SQLite DAL with parameterized queries, UUID IDs. Vector storage via usearch (graceful degradation). Source CRUD, tags, groups, space CRUD and linking, text chunking, embeddings. Token usage tracking. Scoped semantic search with iterative widening. FTS5 keyword search for messages and source chunks
...
- **`services/rag.py`** — RAG pipeline with three retrieval modes (dense, keyword, hybrid). Dense: usearch vector search. Keyword: FTS5 full-text search. Hybrid: reciprocal rank fusion of both. Optional cross-encoder reranking. Space-scoped retrieval, token budget trimming, graceful degradation
...
- **`RerankerConfig`** — Cross-encoder reranker: `enabled` (None=auto-detect), `provider` (local/api), `model`, `top_k` (5), `score_threshold` (0.0), `candidate_multiplier` (3)
