...
- **`services/rag.py`** — RAG pipeline: embed query, search similar messages/source chunks via sqlite-vec, filter by threshold, deduplicate, trim to token budget. Gracefully degrades. `retrieve_context()` accepts `space_id` for scoped retrieval. `RetrievedChunk` includes `conversation_type` field; `format_rag_context()` annotates chunks with type labels (`[note]`, `[doc]`)
...
- **`services/config_overlays.py`** — Pack config overlay collection, merging, and conflict detection. `collect_pack_overlays()` reads `config_overlay` artifacts from DB for attached packs. `merge_pack_overlays()` combines overlays sorted by priority (lower number wins). `detect_overlay_conflicts()` finds dot-path collisions — same priority + same key is an error, different priorities are allowed. `detect_artifact_conflicts()` guards against future exclusive artifact types; currently all non-config types (`skill`, `rule`, `instruction`, `context`, `memory`, `mcp_server`) are additive. Skills use namespace-qualified display names on collision. `flatten_to_dot_paths()` converts nested dicts to flat keys for set-intersection comparison. `track_config_sources()` builds `{dot.path: layer_name}` map for `aroom config view --with-sources`. `check_enforced_field_violations()` pre-flight validation for team-enforced fields
- **`services/pack_sources.py`** — Git-based pack source cache management. Clone, pull, and cache git repos containing pack definitions. URL scheme allowlist (rejects `ext::`, `file://`). Credential sanitization in error messages. Deterministic cache paths via SHA-256. `is_git_url()` detects git URLs (`https://`, `ssh://`, `git@host:path`). Pure sync functions shelling out to `git` binary
...
- **`services/pack_attachments.py`** — Tracks active packs at global/space scopes with priority-based conflict resolution. `attach_pack()` accepts `priority` (1-100, default 50; lower wins). Conflict detection at attach time: config overlay dot-path collisions (same priority = error, different priority = allowed); all non-config artifact types are additive (skills use namespace-qualified display names on collision). `get_active_pack_ids()` and `get_attachment_priorities()` for overlay merge ordering. Syncs with rules/skills loading
...
- **`routers/spaces.py`** — Spaces API: `GET/POST /api/spaces` (list/create), `GET/DELETE /api/spaces/{id}` (show/delete), `GET /api/spaces/{id}/paths` (mapped dirs), `POST /api/spaces/{id}/refresh` (re-parse YAML), `GET/POST/DELETE /api/spaces/{id}/sources` (source linking), `GET /api/spaces/{id}/packs` (attached packs). Each space response includes `origin` field indicating "local" (space-scoped, in `.anteroom/`) or "global" (user-scoped, in `~/.anteroom/spaces/`). Pydantic validation on name pattern and path traversal. Error messages sanitized to prevent path disclosure

#### CLI Modules
- **`cli/repl.py`** — Main REPL loop with prompt_toolkit. Orchestrates: system prompt building, space context, trust verification, plan mode, skill auto-invocation. Delegates slash commands to `commands.py`, agent turns to `agent_turn.py`, events to `event_handlers.py`. Manages `/pack`, `/space`, `/artifact` commands. `/instructions` alias for `/conventions`. Space auto-detection from cwd with parent walk-up. Skill args sanitized with `sanitize_trust_tags()` and delimited with `<skill_args>` tags for injection defense
