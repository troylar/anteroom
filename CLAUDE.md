...
- **`services/rag.py`** — RAG pipeline: embed query, search similar messages/source chunks via sqlite-vec, filter by threshold, deduplicate, trim to token budget. Gracefully degrades. `RetrievedChunk` includes `conversation_type` field; `format_rag_context()` annotates chunks with type labels (`[note]`, `[doc]`)
...
- **`services/spaces.py`** — Space file parser and manager. `SpaceConfig` and `SpaceLocalConfig` frozen dataclasses. `parse_space_file()` with name validation (`^[a-zA-Z0-9][a-zA-Z0-9_-]{0,63}$`), URL scheme validation, 256KB size limit. `validate_space()` checks URLs and paths. `get_space_config_overlay()` extracts config dict. `write_space_file()` / `write_local_file()` for YAML serialization. `list_space_files()` scans `~/.anteroom/spaces/`. `is_local_space(file_path)` detects if a space file is in `.anteroom/` (local/project) vs `~/.anteroom/spaces/` (global). `slugify_dir_name(name)` derives valid space name from directory name. `write_space_template(path, name)` writes a new space YAML template to file
...
- **`cli/layout.py`** — Full-screen terminal layout with HSplit structure: persistent header showing model/directory/git branch, scrolling output pane with mouse-scroll support and auto-scroll-to-bottom, separator lines, status footer, and input area. `OutputControl` auto-scrolls output to latest content via cursor positioning, checkpoint/truncate pattern for in-place re-rendering (used by streaming cursor and plan checklist). `AnteroomLayout` class builds the layout tree. `OutputPaneWriter` provides async write interface for streaming output. `format_header()` renders header text with metadata. `create_anteroom_style()` defines Rich styles for layout elements (including plan checklist styles). Scroll position indicator shows `↑ N lines below` when scrolled up. Approval-mode-aware input prompt colors (gold=auto, green=safe, blue=caution, red=strict) via `set_approval_mode()`
...
- **`cli/renderer.py`** — Rich terminal output: verbosity levels, thinking spinner with lifecycle phases, plan checklist rendering (with fullscreen in-pane rendering via `_render_plan_to_pane()`), inline diff rendering for file tools, tool call dedup, subagent rendering. Fullscreen mode support: `use_fullscreen_output(layout, invalidate_fn)` switches renderer to full-screen mode (redirects all console output to layout's output pane, updates status line instead of raw ANSI manipulation), `is_fullscreen()` detects fullscreen mode to adapt output handling (e.g., suppress cursor manipulation inside OutputPaneWriter). Fullscreen logging: `FullscreenLogHandler` routes Python logging through the renderer's console during fullscreen mode (prevents raw stderr writes from corrupting the prompt_toolkit layout); `install_fullscreen_log_handler()` swaps root logger's stderr handlers, `restore_log_handlers()` restores originals on exit. Visual polish: turn separators (`─── You ───`/`─── AI ───`) via `render_user_turn()`/`render_ai_turn_start()`, box-drawing tool call frames with compact arg summaries, streaming cursor (`▊`) during token output with checkpoint-based truncation for clean re-renders
