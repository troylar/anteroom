...
cli:
  ...
  max_identical_tool_repeats: 3    # Stop if same tool(s) called with identical args N+ times consecutively; 0 to disable (default: 3)
...
| `max_identical_tool_repeats` | integer | `3` | Stop if same tool(s) called with identical arguments N+ times consecutively; 0 to disable |
