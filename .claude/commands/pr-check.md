...
### Step 5c: Documentation Freshness (Sonnet agent)

Launch a Sonnet agent to check whether documentation is up to date with the PR's changes:

1. Get the list of changed files: `cd $DIR && git diff --name-only $BASE..HEAD`
2. Read `$DIR/CLAUDE.md`, `$DIR/README.md`, and `$DIR/VISION.md`
...
- Test count in "Development Commands" significantly wrong? Flag as STALE.
...
📖 Documentation Freshness:
  CLAUDE.md:
    Key Modules:     ✅ / ⚠️ MISSING: <new module not listed>
    Architecture:    ✅ / ⚠️ STALE: <diagram outdated>
    Configuration:   ✅ / ⚠️ MISSING: <new config field>
    Database:        ✅ / ⚠️ MISSING: <new table/column>
    Security Model:  ✅ / ⚠️ STALE: <auth change not reflected>
  ...
  docs/ pages:
    docs/cli/tools.md:          ✅ / ⚠️ STALE: <new tool not documented>
    docs/api/conversations.md:  ✅ / ⚠️ STALE: <endpoint behavior changed>
    docs/configuration/:        ✅ / ⚠️ MISSING: <new config not documented>
```

Rate overall documentation: UP TO DATE / NEEDS UPDATE (list specific files to update).

Documentation issues are **warnings, not blockers** — but strongly recommend updating before merge.

### Step 5d: Vision Alignment (Sonnet agent)
